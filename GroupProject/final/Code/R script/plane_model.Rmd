...
```{r}
count <- table(plane$model) 
delaycount <- table(plane[plane$ArrDelay > 0, "model"]) # delay count for each engine
delayprop <- delaycount/count
delayprop <- as.data.frame(delayprop)
delayprop <- delayprop[which(delayprop$Freq < 1000000),]
names(delayprop)[1] <- 'model'
...
DTsum <- aggregate(ArrDelay ~ model, plane[plane$ArrDelay > 0,], sum)
